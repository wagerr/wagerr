...
CoinWitnessData* CzWGRTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzWGRTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
void CzWGRTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzWGRWallet* zWGRWallet)
{
    bool iszWGRWalletInitialized = (NULL != zWGRWallet);
    CMintMeta meta;
    ...
    if (! iszWGRWalletInitialized)
        zWGRWallet = new CzWGRWallet(strWalletFile);
    meta.isSeedCorrect = zWGRWallet->CheckSeed(dMint);
    if (! iszWGRWalletInitialized)
        delete zWGRWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
std::set<CMintMeta> CzWGRTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzWGRWallet* zWGRWallet = new CzWGRWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zWGRWallet);
        }
        delete zWGRWallet;
