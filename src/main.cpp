...
unsigned int nBettingCacheSize = 1000;
...
bool CheckZerocoinSpend(const CTransaction& tx, bool fVerifySignature, CValidationState& state, bool fFakeSerialAttack)
{
    ...
    for (const CTxIn& txin : tx.vin) {
        ...
        if (isPublicSpend) {
            ...
            if (!ZWGRModule::validateInput(txin, prevOut, tx, ret)){
...

    {
        ...
        if (hasZcSpendInputs) {
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("%s : zWGR spend tx %s already in block %d",
            ...
            // Check that zWGR mints (if included) are not already known
        ...
        bool fCLTVIsActivated = chainActive.Tip()->nHeight >= Params().BIP65Height();
...
void RecalculateZWGRMinted()
{
    ...
    uiInterface.ShowProgress(_("Recalculating minted ZWGR..."), 0);
    while (true) {
        ...
        if (pindex->nHeight % 1000 == 0) {
            ...
            uiInterface.ShowProgress(_("Recalculating minted ZWGR..."), percent);
        }
...
{
    CBlockIndex* pindex = chainActive[Params().Zerocoin_StartHeight()];
    uiInterface.ShowProgress(_("Recalculating spent ZWGR..."), 0);
    while (true) {
        if (pindex->nHeight % 1000 == 0) {
            LogPrintf("%s : block %d...\n", __func__, pindex->nHeight);
            int percent = std::max(1, std::min(99, (int)((double)(pindex->nHeight - Params().Zerocoin_StartHeight()) * 100 / (chainActive.Height() - Params().Zerocoin_StartHeight()))));
            uiInterface.ShowProgress(_("Recalculating spent ZWGR..."), percent);
        }
...
bool RecalculateWGRSupply(int nHeightStart)
{
    ...
    while (true) {
        if (pindex->nHeight % 1000 == 0) {
            ...
            uiInterface.ShowProgress(_("Recalculating WGR supply..."), percent);
        }
...
bool UpdateZWGRSupply(const CBlock& block, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    // A one-time event where only the zWGR supply was off (due to serial duplication off-chain on main net)
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, CBettingsView& bettingsViewCache, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (!fAlreadyChecked && !CheckBlock(block, state, !fJustCheck, !fJustCheck)) {
        if (state.CorruptionPossible()) {
            ...
            // problems.
            return AbortNode("Corrupt block found indicating potential hardware failure; shutting down",
                    _("Corrupt block found indicating potential hardware failure; shutting down"));
        }
        return false;
    }
    ...
    if (fScriptChecks && pindex->pprev) {
        fCLTVIsActivated = pindex->pprev->nHeight >= Params().BIP65Height();
    }
    ...
    //Remove zerocoinspends from the pending std::map
    ...
    if (miPrev != mapBlockIndex.end()) {
        ...
        if (pindexNew->IsProofOfStake()) {
            if (!mapProofOfStake.count(hash))
                LogPrintf("AddToBlockIndex() : hashProofOfStake not found in std::map \n");
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (isPoS) {
        ...
        for (const CTransaction& tx : block.vtx) {
            for (const CTxIn& in: tx.vin) {
                ...
                if(hasWGRInputs) {
                    ...
                    for (const CTxIn& wgrIn : wgrInputs)
        ...
        if (isBlockFromFork) {
            ...
            if(hasZWGRInputs) {
                for (const CTxIn& zWgrInput : zWGRInputs) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(zWgrInput);
...
void static CheckBlockIndex()
{
    ...
    // Build forward-pointing std::map of the entire block tree.
    ...
    // Check that we actually traversed the entire std::map.
