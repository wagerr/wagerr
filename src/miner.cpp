...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake)
{
    ...
    if (fProofOfStake) {
        ...
        if (nSearchTime >= nLastCoinStakeSearchTime) {
            ...
            if (pwallet->CreateCoinStake(*pwallet, pindexPrev, pblock->nBits, nSearchTime - nLastCoinStakeSearchTime, txCoinStake, nTxNewTime, stakeInput)) {
        ...
        if (fProofOfStake) {
            ...
            if (pblock->IsZerocoinStake()) {
                ...
                if (!SignBlockWithKey(*pblock, key)) {
                    LogPrintf("%s: Signing new block with zWGR key failed \n", __func__);
                    return NULL;
                }
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        if (fProofOfStake) {
            ...
            while (vNodes.empty() || pwallet->IsLocked() || !fMintableCoins ||
                   (pwallet->GetBalance() > 0 && nReserveBalance >= pwallet->GetBalance()) || masternodeSync.NotCompleted()) {
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("WGRMiner exception");
    } catch (...) {
        LogPrintf("WGRMiner exception");
    }

    LogPrintf("WGRMiner exiting\n");
}
