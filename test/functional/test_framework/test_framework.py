    ...
    def assert_start_raises_init_error(self, i, extra_args=None, expected_msg=None, *args, **kwargs):
        with tempfile.SpooledTemporaryFile(max_size=2**16) as log_stderr:
            try:
                ...
                assert 'wagerrd exited' in str(e)  # node must have shutdown
    ...
    def _initialize_chain(self):
        ...
        if create_cache:
            ...
            for i in range(MAX_NODES):
                ...
                args = [os.getenv("BITCOIND", "wagerrd"), "-spendzeroconfchange=1", "-server", "-keypool=1", "-datadir=" + datadir, "-discover=0"]
                if i > 0:
...
class ComparisonTestFramework(BitcoinTestFramework):
    ...
    def add_options(self, parser):
        parser.add_option("--testbinary", dest="testbinary",
                          default=os.getenv("BITCOIND", "wagerrd"),
                          help="wagerrd binary to test")
        parser.add_option("--refbinary", dest="refbinary",
                          default=os.getenv("BITCOIND", "wagerrd"),
                          help="wagerrd binary to use for reference nodes (if any)")
