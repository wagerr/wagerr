...
class TestNode():
    ...
    def __init__(self, i, dirname, extra_args, rpchost, timewait, binary, stderr, mocktime, coverage_dir, use_cli=False):
        ...
        if binary is None:
            self.binary = os.getenv("BITCOIND", "wagerrd")
        else:
            self.binary = binary
        self.stderr = stderr
        ...
        self.cli = TestNodeCLI(os.getenv("BITCOINCLI", "wagerr-cli"), self.datadir)
    ...
    def wait_for_rpc_connection(self):
        ...
        for _ in range(poll_per_s * self.rpc_timeout):
            assert self.process.poll() is None, "wagerrd exited with status %i during initialization" % self.process.returncode
            try:
        ...
        raise AssertionError("Unable to connect to wagerrd")
    ...
    def node_encrypt_wallet(self, passphrase):
        ...
        This causes wagerrd to shutdown, so this method takes
