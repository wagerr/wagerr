...
memory available when compiling Wagerr Core. On systems with less, gcc can be
...
Build requirements:
    ...
    sudo apt-get install build-essential libtool autotools-dev autoconf pkg-config libssl-dev libevent-dev bsdmainutils python3 automake
...
**For Ubuntu only:** db4.8 packages are available [here](https://launchpad.net/~wagerr/+archive/ubuntu/ppa).
...
See the section "Disable-wallet mode" to build Wagerr Core without wallet.
...
Disable-wallet mode
--------------------
When the intention is to run only a P2P node without a wallet, wagerr may be compiled in
...
As mentioned above, when maintaining portability of the wallet between the standard Wagerr Core distributions and independently built
