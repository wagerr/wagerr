...
for Ubuntu Xenial does not produce working executables for some of the Wagerr Core applications.
...
Note that for WSL the Wagerr Core source path MUST be somewhere in the default mount file system, for
example /usr/src/wagerr, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
causes two of the wagerr executables to crash shortly after start up. The bug is related to the
...
It's not possible to build the wagerr code using the win32 version of the Mingw-w64 cross compilers (at least not without
modifying headers in the wagerr source code).
