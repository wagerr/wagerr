...
Note that for WSL the Wagerr Core source path should be somewhere in the default mount file system, for
example /usr/src/wagerr, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts may fail.
...
It's not possible to build the Wagerr Core code using the win32 version of the Mingw-w64 cross compilers (at least not without
modifying headers in the Wagerr Core source code).